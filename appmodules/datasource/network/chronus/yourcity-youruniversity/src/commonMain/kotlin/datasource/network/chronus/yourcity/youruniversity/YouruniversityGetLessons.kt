...
suspend fun getLessons(client: HttpClient, json: Json, schedule: Schedule): List<Lesson>? {
	...
	} catch (_: CancellationException) { // ловим отмену операций, причина уже в логе
		null
	}
}

private suspend fun parseTimetable(client: HttpClient, url: String, parseOther: Boolean = true): List<Lesson>? = coroutineScope {
	val parsingScope = this
	...
	return@coroutineScope try {
		withContext(Dispatchers.Default) {
			...
			val otherLessons = if (parseOther) {
				timetableList.getElementsByTag("a").drop(1).map {
					async(Dispatchers.IO) {
						val res = parseTimetable(client, YOUR_PLACE.defaultUrl.dropLast(1) + it.attr("href"), false)
						if (res == null) parsingScope.cancel("Failure to parse another timetable")
						res
					}
				}
			} else emptyList()
			...
			val lessons = days.flatMap { day ->
				...
				if (date.daysUntil(today) <= 7) { // оставляем расписание максимум на неделю назад (если предоставляется)
					day.getElementsByClass("timetable__list-timeslot").flatMap { slot ->
						if (!isActive) throw CancellationException("Failure to parse another timetable")
			...
			lessons + otherLessons.awaitAll().flatMap { it!! }
		}
	} catch (e: Exception) {
